...
**Dấu hiệu thành công:**
- `ps aux | grep java` thấy 2 Java process: 1 WebServer, 1 crawler
...
```bash
# Lấy URL movie có sẵn từ file urls.txt (trên container)
ssh -i ~/.ssh/moviebot_key -p 2222 moviebot@localhost "head -3 /opt/movie-crawler/source/urls.txt"

# Dùng URL đó để test (thay bằng URL thực tế)
TOKEN=$(curl -s -X POST http://localhost:8080/login -H "Content-Type: application/json" -d '{"username":"admin","password":"secret"}' | grep -oP '"token":\s*"\K[^"]+')

curl -s http://localhost:8080/movies?url=https://toivote.com/movie/8beb0593-c5e9-4f38-bcac-5e9c5d63d570 \
...
# Query phim lần 1
URL="https://toivote.com/movie/8beb0593-c5e9-4f38-bcac-5e9c5d63d570"
